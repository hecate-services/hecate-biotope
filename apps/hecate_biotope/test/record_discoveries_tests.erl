...
the_events_are_named_for_what_happened_test() ->
    Snap = snapshot(),
    Names = [maps:get(event_type, E) || E <- all_events(Snap)],
...
a_run_gets_its_own_stream_test() ->
    ...
    ?assertNotEqual(A, B),
    ?assertNotEqual(A, C),
    %% Deterministic: the same island replaying the same seed writes to the same
    %% stream, which is the point of keying on both.
    ?assertEqual(A, discovery:stream_for(<<"beam01">>, 4242)).

%% ⚠ VALIDATED BY THE LIBRARY'S OWN VALIDATOR, NOT BY MY READING OF ITS DOCS.
%%
%% The first stream id was `biotope/beam01/101516166'. reckon-db's contract is
%% `[a-z]{1,32}-[a-f0-9]{32}' and it rejected every append with
%% `invalid_stream_id', after exhausting a retry budget slow enough to block the
%% keeper's own mailbox. Nothing caught it before the fleet did, because nothing
%% asked the thing that decides.
%%
%% `reckon_gater_stream_id' calls itself "the protocol contract ... single source
%% of truth for what a valid stream id means". So this asks it.
a_stream_id_is_one_the_store_will_accept_test() ->
    lists:foreach(
      fun({Island, Seed}) ->
              Id = discovery:stream_for(Island, Seed),
              ?assertEqual(ok, reckon_gater_stream_id:validate(Id)),
              ?assertMatch({user, <<"world">>, _Hex},
                           reckon_gater_stream_id:parts(Id))
      end,
      [{<<"beam01">>, 101516166}, {<<"beam00">>, 1}, {<<"a">>, 0},
       {<<"an-island-with-a-very-long-name-indeed">>, 999999999999}]).
...
a_discovery_says_what_it_means_without_the_code_test() ->
    #{data := Data} = discovery:found(<<"beam01">>, 124, 3000, snapshot()),
...
the_opening_event_carries_the_whole_economy_test() ->
    #{data := Data} = discovery:seeded(<<"beam01">>, snapshot()),
...
every_event_says_when_test() ->
    Snap = snapshot(),
    lists:foreach(fun(E) ->
                          ?assert(is_integer(maps:get(tick, E))),
                          %% AND ITS ISLAND. The stream id is a digest and says
                          %% nothing readable, so without this nothing connects a
                          %% discovery to the island that made it. The comment on
                          %% `event/3' claimed this while the code omitted it.
                          ?assertEqual(<<"beam01">>, maps:get(island, E))
                  end, all_events(Snap)).
...
a_whole_run_is_a_few_hundred_events_test_() ->
    {timeout, 300,
     fun() ->
             W = world:tick(world:new(#{seed => 11, population => 40}), 4000),
...
watched(W, N) ->
    ...
    _Seeded = discovery:seeded(<<"beam01">>, Snap),
    _Ended = discovery:ended(<<"beam01">>, Snap),
...
all_events(Snap) ->
    I = <<"beam01">>,
    [discovery:seeded(I, Snap), discovery:found(I, 7, 40, Snap),
     discovery:settled(I, 900, Snap), discovery:stirred(I, 950, Snap),
     discovery:ended(I, Snap)].
