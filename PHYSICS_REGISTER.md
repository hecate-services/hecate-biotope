...
| F.4 | **Whether a resolving sense is FATAL rather than merely useless** | **OPEN, and the first question here whose answer would change what a world is for.** World 14 made bare ground recover from its neighbours, so a population that grazes its neighbourhood flat loses its own subsidy. A sense that resolves the ground should let creatures find and strip the best cells faster. If that is why every fine scale goes extinct, then perception in this world is not a free good that nothing bothers to buy, it is a liability, and seventeen worlds of asking why brains do not appear have been asking the wrong question. **Offered as a hypothesis and not measured.** The monotone tradeoff in `F.3` is consistent with it and does not establish it. |
