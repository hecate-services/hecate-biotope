...
| **15** | **a mouth is tissue and eating is a decision**: `mouth` is a heritable integer charged by the expression that already prices a frame, `eat` becomes the fourth purpose, and a creature consumes another only if it can and chooses to (`D.2`, `H.1`, targets `G.4`) | _running_ [preregistration](PREREGISTRATION_WORLD15.md) |
