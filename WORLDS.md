...
| **18** | **an act is TISSUE**: an output is charged by its wiring at a swept `act_cost`, so being able to do four things need not cost what being able to do none costs (`H.7`) | _sweeping. **The control is `act_cost` 0, which is world 17's economy exactly**, so an island deployed before the sweep lands runs measured rules rather than a guess. World 17 shipped an unswept value and founded doomed islands for its whole life; this does not repeat that._ [preregistration](PREREGISTRATION_WORLD18.md) |
