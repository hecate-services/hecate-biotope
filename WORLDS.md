...
| **23** | **there is WATER, it is only in some PLACES, and a creature DRIES OUT and must go back to drink**: a store of capacity `structure` draining by a swept `thirst` each tick, refilled to full by standing on a hole, death by drying counted separately as `parched`; a fifth field appended to `?FIELDS` so water can be perceived, and a swept number of holes placed in rings from the centre (`J.1`, `J.2`) | **PENDING, and one finding is already in: THE FIRST DEFAULT WAS AN EXECUTION.** `thirst` 40 and 20 kill **every seed at every hole count**; the world is a tradeoff only between roughly 5 and 10, and the default is **10 at 61 holes** on fewest extinctions alone (7 dead of 12 against the control's 6, with a quarter of all deaths from thirst). ⚠ **61 is the LEAST concentrated arrangement measured and it is what survives**, which is already awkward for `J.1`. The first world gated as an ENVIRONMENT rather than as a trait, after `I.16`. ⚠ **AND THE FIRST DRAFT OF THIS WORLD WAS ABSURD**: it made breeding require standing on water, reasoned from "a creature cannot make a round trip in one life, so thirst is unbuildable" — which does not follow, because a store's leash is `structure / thirst` and both are mine to pick. `I.17`. **⚠ THE FILTER IS THE READING SO FAR.** Three arms at 12 seeds to 2,000 ticks, mean cells to the nearest hole at 61 holes: founders **6.22**, rule OFF **5.86**, rule ON **6.27**. Creatures end up no nearer water than a random scatter and slightly further than with no rule at all, so this is culling by birthplace and not approach. The 3.21-cell figure briefly published as the correction was never measured and is withdrawn. [pre-registration](PREREGISTRATION_WORLD23.md) |
