...
| **19** | **a weight costs only if it is NON-ZERO**: `hidden_weights` and `output_weights` count live weights rather than row length, so a brain pays for what it uses rather than for the shape it was born in (`H.11`, and it re-prices `B.10`) | _sweeping `neural_cost` under the new rule, which is the constant this changes the meaning of_ [preregistration](PREREGISTRATION_WORLD19.md) |
