...
| **16** | **a hidden node is charged by what it READS**: `hidden_weights` replaces `hidden_count` in the tissue expression, so a node wired to six inputs costs six times one wired to a single input, and sensors and brains become coupled (`H.9`, which is `B.3` still standing) | _running_ [preregistration](PREREGISTRATION_WORLD16.md) |
