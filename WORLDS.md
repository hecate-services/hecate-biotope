...
| **17** | **a reading is an INTENSITY, not a total**: a spatial field reports the mean over the cells in reach and the unit becomes `ground_ceiling / reading_ceiling`, so every reach reads on one scale and reach means how far you average rather than how much you add (`F.2`, the third face of `A.6`) | _running_ [preregistration](PREREGISTRATION_WORLD17.md) |
