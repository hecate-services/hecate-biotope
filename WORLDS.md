...
Four hypotheses were refuted on their own pre-registered criteria, including
`J.1`, the register's own central diagnosis. Two structural admissions belong
beside them: the baseline stayed at world 1 while the questions moved to world
20, and **no act ever wrote to the world**, so signalling was inexpressible in
every world above and its absence was never evidence.

What comes next is in [CHARTER.md](CHARTER.md): the world becomes the sum of the
nodes, migration becomes foundational, the body plan is given rather than
re-derived, and a creature can write to a channel.

**Numbering continues below this line rather than restarting**, because the
register cross-references `G.10` and the rest from everywhere and a clean slate
in the numbering would break the one asset worth keeping. The break is marked,
not hidden.
